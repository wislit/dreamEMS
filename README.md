...
```
git clone https://github.com/dreamEMSyXiao/SpringBoot-MyBatis.git
...
- ```src/main/java/com/dreamEMS/constant```: 该目录放置了各种常量类
- ```src/main/java/com/dreamEMS/model```: 该目录放置了各种模型类，其子目录 dto 放置了 DTO（Data Trasfer Object）类，其另一子目录 entity 放置了实体类
- ```src/main/java/com/dreamEMS/monitor```: 该目录放置了各种监测类
- ```src/main/java/com/dreamEMS/repository```: 该目录放置了数据库增删改查的接口，其子目录 impl 放置了这些接口的实现类
- ```src/main/java/com/dreamEMS/service```: 该目录下放置服务（一个服务对应于一些业务逻辑的集合）的接口，其子目录 impl 放置了这些接口的实现类
- ```src/main/java/com/dreamEMS/security```: 该目录放置了 Spring Security OAuth 的相关配置
- ```src/main/java/com/dreamEMS/util```: 该目录放置了各种工具类
- ```src/main/java/com/dreamEMS/web```: 该目录放置了和网络层相关的一切，包括控制器、异常处理、过滤器等等
- ```src/main/resources/com/dreamEMS/repository/mybatis```: 该目录放置了 MyBatis 的映射器 XML 文件
...
```
2017-01-01 13:35:52.698  INFO 13184 --- [nio-8080-exec-1] com.dreamEMS.xxx.XxxClass                   : This is an info log.
2017-01-01 13:35:52.738 ERROR 13184 --- [nio-8080-exec-1] com.dreamEMS.xxx.XxxClass                   : This is an error log.
```
...
public class PerformanceMonitor {
     ...
     * com.dreamEMS.service package or any sub-package under that
     * and modified by public.
     */
    @Pointcut("execution(public * com.dreamEMS.web.controller..*(..))")
...
```
2017-01-03 22:58:19.431 ERROR 6384 --- [nio-8080-exec-9] com.dreamEMS.monitor.PerformanceMonitor     : [BookController.postBook(..)][Elapsed time: 1.457 s][Note that it's time consuming!]
2017-01-03 22:58:47.875  INFO 6384 --- [io-8080-exec-10] com.dreamEMS.monitor.PerformanceMonitor     : [BookController.getBooks(..)][Elapsed time: 0.656 s]
2017-01-03 22:59:16.356  INFO 6384 --- [nio-8080-exec-1] com.dreamEMS.monitor.PerformanceMonitor     : [BookController.putBook(..)][Elapsed time: 0.618 s]
2017-01-03 22:59:51.259  INFO 6384 --- [nio-8080-exec-3] com.dreamEMS.monitor.PerformanceMonitor     : [BookController.deleteBook(..)][Elapsed time: 0.016 s]
```
...
- ```com.dreamEMS.model.dto.CustomUserDetails```: 该类是一个模型类，实现了 ```UserDetails``` 接口。它主要负责传送用户的认证信息，包括：用户名, 密码, 该用户所拥有的权限等等
- ```com.dreamEMS.security.AuthorizationServerConfiguration```: 该类是一个配置类，继承了 ```AuthorizationServerConfigurerAdapter```。它主要负责授权服务器的配置，包括：信任的客户端信息的管理、请求令牌的 URL 的配置、 令牌的管理、如何认证用户的配置、对于请求令牌的 URL 的安全约束的配置等等
- ```com.dreamEMS.security.ResourceServerConfiguration```: 该类是一个配置类，继承了 ```ResourceServerConfigurerAdapter```。他主要负责资源服务器的配置，包括：对于请求资源的 URL 的安全约束的配置等等
- ```com.dreamEMS.security.WebSecurityConfiguration```: 该类是一个配置类，继承了 ```GlobalAuthenticationConfigurerAdapter```。它主要负责有关认证的配置，包括：用户的认证信息的获取等等
- ```com.dreamEMS.service.UserService```: 该类是一个服务类的接口，继承了 ```UserDetailsService``` 接口
- ```com.dreamEMS.service.impl.UserServiceImpl```: 该类是 ```UserService``` 接口的实现类
...
```
curl http://localhost:8080/oauth/token -X POST -u client:fucksecurity -d "grant_type=password&username=dreamEMS&password=fucksecurity"
```
...
在本项目中，第一类情况大量的出现。例如，```com.dreamEMS.web.controller.BookController``` 中的一段代码：
